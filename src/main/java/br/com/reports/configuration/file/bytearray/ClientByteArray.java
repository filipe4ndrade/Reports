...
public class ClientByteArray {
    ...
    private void writeBytesToOutputStream(ResultSet rs, ByteArrayOutputStream outputStream, String... columnNames) throws SQLException, IOException {
        for (String columnName : columnNames) {
            byte[] columnValue = rs.getBytes(columnName);
            outputStream.write(columnValue);
            outputStream.write(',');
        }
        outputStream.write('\n');
    }
    ...
    public ByteArrayOutputStream findById(Integer id) {
        ...
        try {
            ...
            while (rs.next()) {
                writeBytesToOutputStream(rs, outputStream, "id", "name", "identity", "contract", "email", "password", "contract_Number",
                        "street", "number", "city", "state", "complement");
            }
